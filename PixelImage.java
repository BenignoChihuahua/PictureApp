...
public class PixelImage
{
    ...
    public Pixel[][] getPixelArray(PixelImage pi, int pixelRow, int pixelCol)
    {
        Pixel[][] pixelMatrix = new Pixel[3][3];
        Pixel[][] data = pi.getData();
        if(pixelRow == 0 || pixelCol == 0)
            return null;
        if(pixelRow == data.length || pixelCol == data[0].length)
            
        //fill out the rows with the corresponding pixels
        for(int i = 0; i < 3; i++)
            { pixelMatrix[0][i] = data[pixelRow - 1][pixelCol - 1 + i]; }
        
        for(int j = 0; j < 3; j++)
            { pixelMatrix[1][j] = data[pixelRow][pixelCol - 1 + j];     }
            
        for(int k = 0; k < 3; k++)
            { pixelMatrix[2][k] = data[pixelRow + 1][pixelCol - 1 + k]; }
            
        //assign a new 2d 3*3 Pixel array with only the values relative     .
